...
#Ej.2
GoalsGm<-FieldGoals/Games
GoalsGm["DerrickRose","2012"]<-0

#Ej.3 ¿Cuál de todos los jugadores metió más canastas por partido en promedio (todos los años)?
rowMeans(GoalsGm)

max(rowMeans(PntsGm))

#Ej.4 ¿Cuál es el jugador más preciso?
Acc<-FieldGoals/FieldGoalAttempts
Acc["DerrickRose","2012"]<-0
Acc

MeanAcc<-c(rowMeans(Acc))
MeanAcc
RwNms<-c((row.names(Acc))
RwNms

MeanAccMtx<-cbind(MeanAcc)
MeanAccMtx

which(MeanAccMtx==MaxAcc,arr.ind = T)

#Ej.5 ¿Cada cuántos minutos Kobe Bryant mete una canasta en promedio?
MinutesPlayed
MinutesPlayed["DerrickRose","2012"]<-0

MeanMinGm<-rowMeans(MinutesPlayed)/rowMeans(Games)
MeanMinGm
MeanGoalsGm<-rowMeans(GoalsGm)
MeanGoalsGm
MeanMinGm["KobeBryant"]/MeanGoalsGm["KobeBryant"]

#Ej.6 ¿Cuál de todos los jugadores mete más canastas por minuto?

max(MeanGoalsGm/MeanMinGm)
which(MeanGoalsGm/MeanMinGm==max(MeanGoalsGm/MeanMinGm),arr.ind = T)

#Ej.7 Idem 6 desde 2010 a 2014

GoalsGm
MinGm<-MinutesPlayed/Games
MinGm
GoalsMinYr=GoalsGm/MinGm
GoalsMinYr

colnames(GoalsMinYr)

for (col in colnames(GoalsMinYr)){
  print(which(GoalsMinYr==max(GoalsMinYr[,col]),arr.ind = T))
  print(max(GoalsMinYr[,col]))
}
...
legend(1,500, legend=rownames(FieldGoals, pch=15:18,col=c(1:4,6)))
